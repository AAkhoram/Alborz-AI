...
# ================== آموزش ==================
def train(model, data, char_to_ix, ix_to_char):
    ...
    try:
        ...
        # با توجه به تجربه‌ی ۲۴+ ساعت آموزش قبلی، از دست دادن پیشرفت با یک Ctrl+C تصادفی
        # خیلی گرونه - این‌جا قبل از خروج، همون مدل نیمه‌آموزش‌دیده رو ذخیره می‌کنیم
        print("\nآموزش با Ctrl+C متوقف شد - در حال ذخیره‌ی مدل نیمه‌آموزش‌دیده...")
        underlying = model._orig_mod if hasattr(model, "_orig_mod") else model
        save_model(underlying, char_to_ix, ix_to_char)
        print(f"مدل ذخیره شد در {model_path} (می‌تونی باهاش چت کنی، ولی دوباره اجرا کردن اسکریپت از این مدل ادامه نمی‌ده)")
        raise SystemExit(0)
