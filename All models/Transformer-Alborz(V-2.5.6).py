...
def get_batch(data, seq_length, batch_size, device):
    ...
    """
    max_start = len(data) - seq_length  # اصلاح off-by-one: قبلاً -1 اضافه داشت
    if max_start < 1:
        raise ValueError(
...
def _atomic_torch_save(obj, path):
    ...
    اینطوری اگه برق وسط save بره، فایل نهایی نصفه‌ونیمه نمی‌مونه."""
    tmp = path + ".tmp"
    torch.save(obj, tmp)
    os.replace(tmp, path)
...
def train(model, data, char_to_ix, ix_to_char, optimizer=None, start_epoch=0, start_step=0,
    ...
    try:
        for epoch in range(start_epoch, epochs):
            # FIX 1: اگه از وسط epoch ادامه می‌دیم، فقط stepهای باقی‌مونده رو اجرا کن
            ...
            # در پایان هر epoch checkpoint ذخیره می‌کنیم (epoch+1 یعنی epoch بعدی)
...
def save_model(model, char_to_ix, ix_to_char, trained_epochs=None):
    """trained_epochs: چند epoch واقعاً آموزش دیده (برای تشخیص افزایش epochs توسط کاربر)"""
...
def _maybe_compile(model, data):
    """torch.compile رو روی مدل اعمال می‌کنه (برای fresh training و resume مشترک)."""
    if not use_compile:
        return model
    try:
...
if __name__ == "__main__":
    ...
    if os.path.exists(checkpoint_path):
        # ===== رزوم از checkpoint (با optimizer state کامل) =====
        ...
        if start_epoch >= epochs:
            ...
            # FIX 3: حتی وقتی همه epochها تموم شده، model.pt رو از checkpoint بساز
            underlying = model._orig_mod if hasattr(model, "_orig_mod") else model
