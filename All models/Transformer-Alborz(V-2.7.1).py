...
# ================== لود دیتاست ==================
def load_data(path: str = "dataset") -> Tuple[str, List[str], Dict[str, int], Dict[int, str]]:
    """
...
def get_batch(
    data: torch.Tensor, seq_length: int, batch_size: int, device: torch.device
...
def _maybe_compile(model: nn.Module, data: torch.Tensor) -> nn.Module:
    if not use_compile:
        return model
    try:
