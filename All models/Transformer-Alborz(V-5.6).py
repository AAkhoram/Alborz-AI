...
#   32000 → دقیق‌تر
#   50000 → نزدیک به GPT-2
vocab_size = 8000

# ================== شخصیت ==================
# System prompt — این متن قبل از هر پیام کاربر به مدل داده می‌شه.
# کوتاه نگهش دار چون context محدوده (seq_length=64 توکن)
...
# ================== LoRA تنظیمات ==================
# lora_rank: رتبه ماتریس کم‌رتبه. بالاتر = انعطاف‌پذیرتر ولی سنگین‌تر
# برای شخصیت "البرز"، یه adapter با اسم "elborz" بساز و finetune کن روی
# دیتاست مکالمه‌ای با سبک مودب/صبور.
#   4   → سبک‌ترین، برای سبک‌های ساده
#   8   → تعادل (توصیه‌شده)
#   16  → پیچیده‌تر، برای شخصیت‌های عمیق
#   64  → نزدیک به full-finetune ولی هنوز سبک
lora_rank = 4

# lora_alpha: ضریب مقیاس. معمولاً 2×rank
lora_alpha = 16

# lora_dropout: روی لایه‌های LoRA
lora_dropout = 0.05

# آیا LoRA فعال باشه؟ False = آموزش عادی (full finetune)
use_lora = True

# پوشه ذخیره‌ی adapter های LoRA
adapters_dir = "adapters"
os.makedirs(adapters_dir, exist_ok=True)

# adapter فعال فعلی (برای چت/آموزش)
...
class LoRALinear(nn.Module):
    """
    wrapper روی nn.Linear که LoRA اضافه می‌کنه.
    ...
    def __init__(
        ...
        # A: رندوم init (Kaiming)
        self.lora_A = nn.Parameter(torch.zeros(in_features, r))
        # B: صفر init → شروع بدون تغییر
    ...
    def merge(self) -> nn.Linear:
        """
        adapter رو merge می‌کنه توی لایه اصلی و یه Linear خالص برمی‌گردونه.
        برای inference سریع‌تر بدون overhead LoRA.
        """
        merged = nn.Linear(
    ...
    def get_lora_state_dict(self) -> Dict[str, torch.Tensor]:
        """فقط وزن‌های LoRA رو برمی‌گردونه (برای save adapter)."""
        return {
    ...
    def load_lora_state_dict(self, state: Dict[str, torch.Tensor]) -> None:
        """فقط وزن‌های LoRA رو لود می‌کنه."""
...
class TinyTransformer(nn.Module):
    def __init__(
        ...
        if not use_lora:
            ...
            # توی LoRA نمی‌تونیم weight tying داشته باشیم چون head یه wrapper شده
            # ولی می‌تونیم init head رو از token_embed کپی کنیم
    ...
    def get_lora_params(self) -> List[nn.Parameter]:
        """فقط پارامترهای LoRA رو برمی‌گردونه (برای optimizer)."""
        lora_params = []
    ...
    def merge_adapter(self) -> None:
        """
        adapter فعلی رو merge می‌کنه توی وزن‌های اصلی.
        بعدش می‌تونی use_lora=False کنی و سریع‌تر inference بگیری.
        WARNING: این عملیات برگشت‌ناپذیره (برای base model).
        """
        ...
        for module in self.modules():
            if isinstance(module, LoRALinear):
                # replace parent attribute
                # این پیچیده‌ست چون need to replace in parent module
                pass
...
def configure_optimizer(
    ...
    if use_lora:
        # فقط LoRA params آموزش می‌بینن، base frozen
...
def chat_loop(
    ...
    if model.use_lora:
        ...
        if adapters:
            ...
            print("برای سوییچ: /adapter نام_اداپتر")
        else:
            print("هیچ adapter ذخیره‌شده‌ای نیست.")
    print("=" * 50)
    ...
    while True:
        ...
        if user_input.startswith("/adapter "):
            ...
            if not model.use_lora:
                print("مدل LoRA نداره که adapter سوییچ کنی.")
                continue
            try:
                model.load_adapter(adapter_name)
        ...
        # اضافه کردن system prompt قبل از ورودی کاربر
