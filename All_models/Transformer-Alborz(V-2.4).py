...
# ================== تنظیمات ==================
d_model = 512
n_layers = 16          # با Pre-LN (پایین‌تر توضیح داده شده) این عمق دیگه دچار vanishing gradient نمی‌شه
n_heads = 8
d_ff = 1024
seq_length = 128
batch_size = 16
...
# نکته: چون torch.compile فقط برای CPU نصب می‌شه (بدون CUDA)، این خط همیشه cpu برمی‌گردونه؛
# بی‌خطره و برای سازگاری با سیستم‌های دیگه نگه داشته شده
...
def train(model, data):
    steps_per_epoch = len(data) // seq_length
