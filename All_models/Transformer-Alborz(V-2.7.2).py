...
def get_batch(
    ...
    از چسبوندن همه‌ی فایل‌ها). هر عضو batch از یک فایل که تصادفی و با احتمال
    متناسب با طولش انتخاب می‌شه، نمونه‌برداری می‌کنه - این‌طوری فایل‌های بزرگ
    به همون نسبت طولشون بیشتر دیده می‌شن، ولی فایل‌های کوچیک هم توی مخلوط
    گم نمی‌شن و در حافظه هم فایل‌ها جدا از هم می‌مونن (بدون concat کردن).
    ...
    file_ix = torch.multinomial(file_weights, batch_size, replacement=True)

    xs, ys = [], []
    for fi in file_ix.tolist():
        d = usable[fi]
